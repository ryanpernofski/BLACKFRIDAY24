...
// Toggle Framing Options Function
function toggleFramingOptions() {
    var framingOptions = document.getElementById("framing-options");
    var toggleIcon = document.getElementById("toggle-icon");
    if (framingOptions.style.display === "none" || framingOptions.style.display === "") {
...
document.addEventListener('DOMContentLoaded', function() {
  console.log('Custom framing script loaded'); // Add this line
  // Attach event listener to the toggle button
    const toggleButton = document.getElementById("toggle-framing");
    if (toggleButton) {
        toggleButton.addEventListener("click", toggleFramingOptions);
    }

    // Add click event listeners to each variant-pill button
    document.querySelectorAll('.variant-pill').forEach(button => {
        button.addEventListener('click', function() {
            // Remove 'selected' class from all buttons
            document.querySelectorAll('.variant-pill').forEach(btn => btn.classList.remove('selected'));

            // Add 'selected' class to the clicked button
            this.classList.add('selected');

            // Store the selected variant ID in the hidden input
            const variantId = this.getAttribute('data-variant-id');
            const selectedVariantInput = document.getElementById('selected-variant-id');
            if (selectedVariantInput) {
                selectedVariantInput.value = variantId;
            } else {
                console.error("Cannot find selected-variant-id element.");
            }
        });
    });

    // Add event listener to Add to Cart button to include framing selection
    document.querySelectorAll('form.product-form').forEach(form => {
        form.addEventListener('submit', function(event) {
           ...
           console.log('Form submission intercepted'); // Add this line
            ...
            let mainVariantId;
            const variantInput = form.querySelector('input[name="id"]:checked') || form.querySelector('input[name="id"]') || form.querySelector('select[name="id"]');
            if (variantInput) {
                mainVariantId = variantInput.value;
            } else {
                console.error('Cannot find main product variant ID');
                return;
            }

            // Get the selected framing variant ID
            const selectedVariantId = document.getElementById('selected-variant-id').value;

            // Get the quantity selected by the user (default to 1 if not found)
            ...
            // Send the POST request to add items to the cart
            fetch('/cart/add.js', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
            ...
            .then(data => {
                // Redirect to cart or update the UI accordingly
                window.location.href = '/cart';
            })
            .catch(error => {
                console.error('Error adding items to cart:', error);
                alert('Sorry, there was an error adding the product to the cart. Please try again.');
            });
        });
    });
});
